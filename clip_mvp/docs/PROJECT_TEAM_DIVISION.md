...
最终成片阶段的字幕排版、SRT/ASS 导出、字幕烧录和字幕与配音时间轴对齐也归成员 B 负责。成员 A 负责 ASR 字幕进入文案阶段，成员 B 负责最终视频里的成片字幕。
...
- 维护 FFmpeg 剪切、拼接、旁白混音、字幕烧录、SRT、ASS、EDL 输出。
- 维护成片字幕策略，包括单行字幕、语义优先拆分、字号缩放、底部居中和 ASS 样式生成。
- 维护字幕与最终配音时间轴对齐，避免字幕提前或滞后于旁白。
...
- FFmpeg 渲染需要处理原声保留、旁白混音、字幕烧录、尾音补齐和输出文件生成。
- 字幕不能简单按固定字数截断，需要在单行展示、语义完整和屏幕可读性之间平衡。
- 最终字幕应优先跟随配音时间轴，而不是只跟随原视频粗剪时间轴。
...
- 系统可按配音时长完成 LLM 选片。
- 系统可生成 MP4、SRT、ASS、EDL。
- 系统可将单行字幕烧录进最终 MP4，字幕底部居中并按视频高度缩放字号。
- 成片字幕能按最终配音时间轴生成，避免旁白和字幕错位。
...
- 输出视频可播放，旁白不被截断。
- 输出视频包含烧录字幕，字幕单行显示、底部居中，且与旁白节奏基本一致。
- matched segments 数量与 beats 一致。
- 前端能展示最终成片、SRT、ASS、EDL 和历史方案。
- 新建配音模板后能在任务创建表单中选择。
...
| B | 配音选片到成片输出 | TTS、LLM 选片、FFmpeg、字幕烧录、输出 | 结果预览、片段展示、配音配置 | 高 | 高 |
...
- 用户可以上传视频创建 AI 解说任务。
- 系统可以生成文案初稿并进入人工审核。
- 用户确认文案后系统可以生成配音、匹配素材、烧录字幕并渲染成片。
- 最终结果可预览和下载。
- 任务事件日志能展示主要阶段耗时和结果。
